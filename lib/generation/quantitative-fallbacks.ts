...
export function selectQuantitativeFallback(
  ...
  // If still nothing, try without prompt deduplication
  if (available.length === 0) {
    available = fallbacks.filter((fb) => fb.canUse(ctx))
  }
  ...
  if (selected.format === "ab") {
    ...
    return {
      ...
      signals: {
        ...
        margin: 0.5, // Moderate margin for quantitative fallbacks
        volatility: 0,
      },
      explainData,
